...
chrome.webRequest.onCompleted.addListener((info) => {
  let match = info.url.match(/[0-9a-f]+\.jpeg/);
  if( !match )
    return

  if( got_images[match[0]] )
    return
  ...
  xhr.onreadystatechange = () => {
    ...
    xhr.onreadystatechange = () => {}
    if( xhr.status==200 ){
      if( xhr.response.size > 100000 ){
        console.log('stored', match[0]);
        got_images[match[0]] = xhr.response
      }
      else
        console.log('too small, skipped', match[0])
    }
    else
      console.log('fetch failed', match[0])
  }
  xhr.open('GET', info.url)
  xhr.send()

}, {urls: ["https://readstorage.bookwalker.com.tw/*"], types: ['image']})

let chunks = []
chrome.runtime.onMessage.addListener((msg, sender, sendRes) => {
  if( msg.act === 'get_entry_count' ){
    let keys = []
    for(let url in got_images)
      keys.push(url)
    sendRes(keys.length)
  }
  if( msg.act === 'download' ){
    let tar = new tarball.TarWriter()
    for(let filename in got_images)
      tar.addFile(filename, got_images[filename])
    got_images = {}
    ...
    tar.write().then((blob) => {
      ...
      if( 65536 < blob.size ){
        reader.onload = () => {
          ...
          i += 65536
          if( i+65536 < blob.size ){
            reader.readAsBinaryString(blob.slice(i, i+65536))
          }
          else{
            reader.onload = () => {
              chunks.push(reader.result)
              sendRes(chunks.length)
            }
            reader.readAsBinaryString(blob.slice(i, blob.size))
          }
        }
        reader.readAsBinaryString(blob.slice(0, 65536))
      }
      else{
        reader.onload = () => {
          sendRes([reader.result])
        }
